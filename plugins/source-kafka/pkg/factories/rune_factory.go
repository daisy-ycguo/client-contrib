...
func (f *kafkaSourceRunEFactory) UpdateRunE() sourcetypes.RunE {
	return func(cmd *cobra.Command, args []string) error {
		...
		source, err := f.kafkaSourceClient.GetKafkaSource(name)
		if err != nil {
			return err
		}
		if source.GetDeletionTimestamp() != nil {
			return fmt.Errorf("can't delete kafka source %s because it has been marked for deletion", name)
		}

		b := client.NewKafkaSourceBuilderFromExisting(source)

		if cmd.Flags().Changed("servers") {
			b.BootstrapServers(f.kafkaSourceFactory.KafkaSourceParams().BootstrapServers)
		}

		if cmd.Flags().Changed("topics") {
			b.Topics(f.kafkaSourceFactory.KafkaSourceParams().Topics)
		}

		if cmd.Flags().Changed("consumergroup") {
			b.ConsumerGroup(f.kafkaSourceFactory.KafkaSourceParams().ConsumerGroup)
		}

		if cmd.Flags().Changed("sink") {
			return fmt.Errorf(
				"cannot update Kafka '%s' in namespace '%s' "+
					"because sink is immutable", name, f.kafkaSourceClient.Namespace())
		}

		err = f.kafkaSourceClient.UpdateKafkaSource(b.Build())
		if err != nil {
			return fmt.Errorf(
				"cannot update KafkaSource '%s' in namespace '%s' "+
					"because: %s", name, f.kafkaSourceClient.Namespace(), err)
		}

		if err == nil {
			fmt.Fprintf(cmd.OutOrStdout(), "Kafka source '%s' updated in namespace '%s'.\n", args[0], f.kafkaSourceClient.Namespace())
		}
...
func (f *kafkaSourceRunEFactory) DescribeRunE() sourcetypes.RunE {
	return func(cmd *cobra.Command, args []string) error {
		...
		writeKafkaSource(dw, kafkaSource, printDetails)
		dw.WriteLine()
		...
		commands.WriteConditions(dw, kafkaSource.Status.Conditions, printDetails)
		if err := dw.Flush(); err != nil {
			return err
		}
		return nil
...
func writeKafkaSource(dw printers.PrefixWriter, source *v1alpha1.KafkaSource, printDetails bool) {
